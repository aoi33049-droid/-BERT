...
class Relative_Pronoun:
    ...
    def __init__(self, text, root_depth,nlp,unmasker_model):
        self.text = text
        self.root_depth = root_depth
        self.nlp = nlp 
        self.unmasker = unmasker_model
        ...
        for words in doc[1:]:        
            if words.dep_ in ["relcl", "acl"] and not words.pos_ == "NOUN": 
                ...
                if not maker and has_subject :
                    ...
                    if not has_mark:
                        start_token = words.left_edge
                        idx_i = start_token.i
                        
                        
                        # doc.text 内での正確な文字位置（インデックス）を取得
                        insert_idx = start_token.idx
                        
                        # 節の直前に [MASK] を挿入して文字列を作成
                        # 例: "The book I read" -> "The book [MASK] I read"
                        original_text = doc.text
                        masked_text = original_text[:insert_idx] + "[MASK] " + original_text[insert_idx:]
                        
                        # ---------------------------------------------------------
                        # 2. BERTによる推論 (ご提示のロジック)
                        # ---------------------------------------------------------
                        rel_type_result = "関係代名詞(省略形)" # デフォルト値

                        try:
                            # top_k=5 で上位5つの候補を取得
                            predictions = self.unmasker(masked_text, top_k=2)
                            
                            # 候補の単語リストを作成（小文字化）
                            candidate_tokens = [p['token_str'].lower().strip() for p in predictions]
                            ...
                            # --- 判定ロジックの変更 ---
                            # 優先順位1: 関係副詞 (where, when, why, how)
                            # 理由: thatよりも具体的であり、候補にあるならそちらの性質が強いため
                            for token_str in candidate_tokens:
                                if token_str in ["where", "when", "why", "how"]:
                                    rel_type_result = f"関係副詞(省略形・{token_str})"
                                    found_rel = True
                                    if idx_i >= 2:
                                        prev_1 = start_token.doc[idx_i - 1].lower_ # 1つ前の単語 (例: way)
                                        prev_2 = start_token.doc[idx_i - 2].lower_ # 2つ前の単語 (例: the)
                                        
                                        if prev_1 == "way" and prev_2 == "the":
                                            found_rel = False
                                            rel_type_result = "関係代名詞(省略形)" # デフォルトに戻す（必要に応じて）
                                    
                                    break # 副詞が見つかったらループを抜ける
                            
                            # 優先順位2: 関係代名詞 (that, which, who, whom)
                            # 副詞が見つからなかった場合のみ、こちらをチェック
                            if not found_rel :
                                for token_str in candidate_tokens:
                                    if token_str in ["that", "which", "who", "whom"]:
                                        rel_type_result = "関係代名詞(省略形)"
                                        found_rel = True
                                        break
                            
                            # 上位候補に決定打がない場合のフォールバック
                            if not found_rel:
                                rel_type_result = ""
                        ...
                        # 結果テキストを作成（BERTの判定結果を含める）
                        if(rel_type_result != ""):
                                result_str = f"{rel_type_result}, 修飾種別 => {Syuusyoku_TEXT4}"
                                end_token.append(result_str)
                                result_token.append((words.i, result_str))
...
def main(testdata,nlp,unmasker):
    print("-" * 30)
    print(f"原文: {testdata}")
    # クラスのインスタンス化
    ...
    # 解析実行
    test_text_serch = Relative_Pronoun(testdata, root_depth,nlp,unmasker)
